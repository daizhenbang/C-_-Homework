...
public:
    ...
    double spirit;
    dragon(){}
    dragon(std::string _name, int _life, int _weapon, double _spirit):warrior(_name,_life),weapon(_weapon),spirit(_spirit){}
    dragon(const warrior& w, int _weapon, double _spirit):warrior(w),weapon(_weapon),spirit(_spirit){}
    ...
    void printExra(std::string weaponName){
        double reSpirit = spirit*100;
        if( (reSpirit-floor(reSpirit)) > (ceil(reSpirit)- reSpirit)) reSpirit = ceil(reSpirit);
        else reSpirit = floor(reSpirit);
